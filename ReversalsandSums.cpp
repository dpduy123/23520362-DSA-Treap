...
struct Node {
	...
	int sum;
	Node() {}
	Node(int val) : val(val), size(1), weight(getpri()), inv(0), sum(val), left(NULL), right(NULL){};
} *root;

int size(Node *cur) {return cur ? cur->size : 0;}
int sum(Node *cur) {return cur ? cur->sum : 0;}
...
void split(Node *treap, Node *&left, Node *&right, int key, int add = 0) {
	...
	treap->sum = sum(treap->left) + sum(treap->right) + treap->val;
}
